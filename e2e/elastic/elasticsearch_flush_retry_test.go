...
// TestBatcherWithElasticsearch_FlushRetriesThenSucceeds updates a document
// that does not exist yet. Elasticsearch reports a document_missing_exception
// on the first attempt; the document is created while the batcher is waiting
// for its retry, so the retried flush succeeds against the real cluster.
func TestBatcherWithElasticsearch_FlushRetriesThenSucceeds(t *testing.T) {
	...
	if err := client.Bulk(ctx, createPayload); err != nil {
		t.Fatal("e2e; elasticsearch; failed to create document for retry scenario:", err)
	}
	...
	if err != nil {
		t.Fatal("e2e; elasticsearch; failed to check document existence:", err)
	}
	if !exists {
		t.Fatal("e2e; elasticsearch; expected document to exist after retried update")
	}
}

// TestBatcherWithElasticsearch_FlushExhaustsRetriesOnPersistentFailure updates
// a document that is never created, so every attempt fails against the real
// cluster. Flush must retry exactly MaxRetries times and then return the error.
func TestBatcherWithElasticsearch_FlushExhaustsRetriesOnPersistentFailure(t *testing.T) {
	ctx := context.Background()
	...
	if err == nil {
		t.Fatal("e2e; elasticsearch; expected flush to fail after exhausting retries")
	}
